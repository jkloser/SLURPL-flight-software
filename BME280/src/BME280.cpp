...
BME280::BME280(){
	...
	settings.sensorMode = 0b11;
	settings.IIRfilter;
	settings.tempOversampling;
	settings.pressOversampling;
	settings.humidOversampling;
	settings.tStandby = 0b000;
}
...
void BME280::writeConfig(void){
	...
	value = ((settings.IIRfilter << 2) | (settings.tStandby << 5))& 0b11111100;
...
void BME280::setMeas(void){
	...
	byte value;
	value = settings.humidOversampling & 0b00000111;
	writeByte(BME280_ctrl_hum, value);

	//set Temp, Press Oversampling, sensor mode
	value = (settings.tempOversampling << 5) & 0b11100000;
	value |= (settings.pressOversampling << 2) & 0b00011100;
	value |= (settings.sensorMode) & 0b00000011;
...
void BME280::readAll(BME280_Measurements *measurements){
	...
	if (var1!=0){
		...
		var2 = (((int64_t)bme280_coefficients.dig_P8) * P) >> 19;
		P = ((P + var1 + var2) >> 8) + (((int64_t)bme280_coefficients.dig_P7)<<4);
		P = P/256.0; //Exception handling when dviding by zero
	}
	else{
		P = 0;
	}
...
uint8_t BME280::readByte(byte reg){
	...
	Wire.requestFrom(settings.I2CAddress, (uint8_t)1);
	value = Wire.read();
	return value;
}
